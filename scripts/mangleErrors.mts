 ...
 *
 * ```ts
 * import type { Id } from "./typeHelpers.js";
 *
 ...
 * fn(someInterface as Id<SomeInterface>); // ✅ Good: transform an `interface` into a `type`
 * ```
 *
 ...
 * @internal
 */
export type Id<BaseType> = BaseType extends (...args: never[]) => unknown
  ? BaseType
  : NonNullable<unknown> & {
...
const formatProdErrorMessageAbsoluteFilePath = path.join(
  import.meta.dirname,
  // FIXME: This is the wrong path, but it works for now. We should figure out a way to fix this.
  // '..',
  'src',
  'utils',
